...
public class Candidate {
     ...
     */
    public Candidate(String rawCandidate, String template, String annotation, List<String> actualNums, boolean isUserDict) {
    ...
    /**
     * テンプレート内の数値置換プレースホルダ (#0〜#9) を実際の数値で置換します。
     * <p>
     * プレースホルダが現れる順に、見出し語から抽出された数値リストの各値が割り当てられます。
     * 数字の後の 1 文字は変換タイプ（0: そのまま, 1: 全角, 2: 漢数字 等）を指定します。
     * </p>
     *
     * @param template 置換対象のテンプレート（例: "#1年#1月#1日"）
     * @param nums     置換に使用する生の数値文字列リスト（例: ["2024", "4", "1"]）
     * @return プレースホルダが解決された後の文字列
     */
