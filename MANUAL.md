...
- **辞書ツール・外部SKK辞書の独立管理**:
 - 本アプリでは「**ユーザー学習辞書**（日常の変換・学習結果）」と「**インポート辞書**（`SKK-JISYO.L` 等の追加辞書）」をそれぞれ独立した別データベースとして分離管理しています。
 - **定番SKK辞書のダウンロード (Web)**: 辞書ツールのメニューから「定番SKK辞書をダウンロード (Web)」を選択すると、`SKK-JISYO.L`, `SKK-JISYO.M`, `SKK-JISYO.jinmei` 等の公式辞書ファイルを外部ブラウザで簡単に開いてダウンロードできます。
 - **外部SKK辞書のインポート**: ダウンロードした SKK 辞書ファイル（`SKK-JISYO.L` など）を端末ローカルからインポートして検索対象に追加できます。学習辞書とは分離されているため、学習履歴を汚さずに巨大な辞書を追加利用でき、クリアや再インポートも個別におこなえます。文字コード（EUC-JP / UTF-8）の自動判定および進捗表示に対応しています。
